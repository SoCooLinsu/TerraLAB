...
class TerraSurrogateNet(nn.Module):
    def __init__(self, in_features=4, num_centers=2048, out_features=5):
        super().__init__()
        self.in_features = in_features
        self.num_centers = num_centers  # 공간에 뿌려둘 기준점(Center)의 개수
        self.out_features = out_features

        # 🌟 1. Centers (기준점): 0~1 사이로 초기화되며, 학습을 통해 최적의 위치로 이동함
        self.centers = nn.Parameter(torch.rand(num_centers, in_features))
        
        # 🌟 2. Gamma (퍼짐 정도): 종 모양(Bell curve)의 뾰족한 정도. 학습 가능.
        self.gamma = nn.Parameter(torch.ones(num_centers) * 5.0)
        
        # 🌟 3. Weights: 각 Center에서 계산된 거리(영향력)를 최종 5개의 물리량으로 변환하는 선형 가중치
        self.linear = nn.Linear(num_centers, out_features)

    def forward(self, x):
        # x 차원: [Batch, 4]
        # 거리 계산을 위해 차원 확장 (Broadcasting)
        x_exp = x.unsqueeze(1)               # [Batch, 1, 4]
        c_exp = self.centers.unsqueeze(0)    # [1, Num_centers, 4]
        
        # 유클리디안 거리의 제곱 계산: ||x - c||^2
        dist_sq = ((x_exp - c_exp) ** 2).sum(dim=-1)  # [Batch, Num_centers]
        
        # RBF 함수 통과: 가우시안 폼 exp(-gamma * r^2)
        rbf_out = torch.exp(-self.gamma * dist_sq)    # [Batch, Num_centers]
...
if __name__ == "__main__":
    ...
    # 🌟 [수정됨] RBF 모델 선언
    num_rbf_centers = 2048 # 늘릴수록 정확해지지만 연산량이 증가함
    model = TerraSurrogateNet(in_features=4, num_centers=num_rbf_centers, out_features=5).to(device)
    
    # 💡 [RBF 꿀팁] Center 초기화: 랜덤 난수보다 실제 학습 데이터 중 일부를 
    # Center의 초기 위치로 박아두면 수렴 속도가 기가 막히게 빨라집니다.
    with torch.no_grad():
        random_indices = torch.randperm(len(train_dataset))[:num_rbf_centers]
        initial_centers = train_dataset.tensors[0][random_indices] # X 데이터에서 추출
        model.centers.copy_(initial_centers)
    
    optimizer = optim.Adam(model.parameters(), lr=0.01) # MLP보다 초기 LR을 살짝 높여줍니다
